...
class WarpAUVEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        forward_scaled = forward_raw * 0.5
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        if self._debug and len(env_ids) > 0:
            ...
            if abs(alignment_check - 1.0) > 0.01:
                print("❌ ALIGNMENT ERROR at reset!")
            else:
                print("✅ ALIGNMENT OK at reset")

        if self._debug and len(env_ids) > 0:
            print(f"=== RESET DEBUG ===")
            print(f"Alignment at reset: {alignment_check:.3f}")
            print(f"Initial angle difference: {math.degrees(angle_diff.item()):.1f}°")
...
# TorchScript REWARD (JIT)
# =======================
# NOTE: This version assumes the vehicle's FORWARD axis is +X in the BODY frame
# =======================
# TorchScript REWARD (JIT)
# =======================
# NOTE: This version assumes the vehicle's FORWARD axis is +X in the BODY frame
...
def _compute_circular_path_reward(
    ...
    # Direction alignment (BODY +X is forward)
