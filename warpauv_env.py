"""
WarpAUV environment for IsaacLabs - Circular Path Following (ENHANCED ROBUSTNESS)
...
- ENHANCED: Improved robustness across all environments with stronger penalties and responsive control.
...
class WarpAUVEnvCfg(DirectRLEnvCfg):
    ...
    num_actions = 2
    num_observations = 23  # ENHANCED: Added 3 new observations
    num_states = 0
    ...
    observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(23,), dtype=np.float32)
    ...
    # ENHANCED: Stronger reward scales for better robustness
    rew_scale_position: float = 12.0
    rew_scale_speed: float = 6.0
    rew_scale_direction: float = 3.0  # Increased from 6.0
    ...
    # ENHANCED: Drift termination threshold
    max_heading_error_degrees: float = 45.0
...
class WarpAUVEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: WarpAUVEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # ENHANCED: Performance tracking for robustness diagnostics
        self._env_performance = torch.zeros(self.num_envs, device=self.device)
        self._prev_heading_error = torch.zeros(self.num_envs, device=self.device)
        ...
        # ENHANCED: More responsive action smoothing
        self._prev_actions = torch.zeros(self.num_envs, 2, device=self.device)
        self.action_smoothing = 0.3  # Reduced from 0.7 for faster response
        self.max_action_change = 0.3

        print("=== ENHANCED CIRCULAR PATH FOLLOWING CONFIGURATION ===")
        ...
        print(f"Max heading error: {self.cfg.max_heading_error_degrees}°")
        print(f"Enhanced direction reward scale: {self.cfg.rew_scale_direction}")
    ...
    def _compute_angle_differences(self) -> torch.Tensor:
        """Compute heading angle differences for drift detection."""
        circle_props = self._get_circle_properties(self._robot.data.root_pos_w)
        
        # AUV forward direction in world frame
        auv_forward_world = quat_apply(
            self._robot.data.root_quat_w, 
            torch.tensor([1.0, 0.0, 0.0], device=self.device).repeat(self.num_envs, 1)
        )
        
        # Desired tangent direction
        desired_tangent = circle_props["desired_tangent_world"]
        
        # Calculate angle between them
        angle_diff = torch.atan2(
            auv_forward_world[:, 0] * desired_tangent[:, 1] - auv_forward_world[:, 1] * desired_tangent[:, 0],
            auv_forward_world[:, 0] * desired_tangent[:, 0] + auv_forward_world[:, 1] * desired_tangent[:, 1]
        )
        
        return torch.abs(angle_diff)
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        forward_scaled = forward_raw * 0.55
        yaw_raw = torch.clamp(actions[:, 1], -1, 1)
        yaw_scaled = yaw_raw * 0.4

        # ENHANCED: More responsive smoothing
        alpha = 0.1
        ...
        # ENHANCED: Stronger yaw control authority
        self._full_actions[:] = 0.0
        self._full_actions[:, 1] = forward_final            # drive_right
        self._full_actions[:, 2] = yaw_final * 0.6        # rear_left (increased from 0.8)
        self._full_actions[:, 3] = yaw_final * 0.6        # rear_right (increased from 0.8)

        # Move debug print AFTER variable assignments
        if self._debug:
            print("=== ENHANCED THRUSTER MAPPING ===")
            print(f"Forward action: {forward_final[0]:.3f} -> drive_right thruster: {self._full_actions[0,1]:.3f}")
            print(f"Yaw action: {yaw_final[0]:.3f} -> rear_left: {self._full_actions[0,2]:.3f}, rear_right: {self._full_actions[0,3]:.3f}")
            print(f"Enhanced yaw authority: 1.2x (was 0.8x)")
    ...
    def _get_observations(self) -> dict:
        ...
        # Desired tangent (WORLD->BODY) - FIXED to match debug calculation
        ...
        # Use the same quaternion method as debug calculation
        ...
        # Direction alignment: BODY +X vs BODY tangent - FIXED coordinate frame
        ...
        # ENHANCED: Additional observations for robustness
        angle_errors = self._compute_angle_differences()
        heading_error_magnitude = angle_errors
        
        # Heading error rate (derivative for prediction)
        dt = self.sim.cfg.dt * self.cfg.decimation
        heading_error_rate = (heading_error_magnitude - self._prev_heading_error) / dt
        self._prev_heading_error = heading_error_magnitude.clone()
        
        # Time-weighted alignment consistency
        alignment_consistency = torch.exp(-5.0 * heading_error_magnitude)
        ...
        obs = torch.cat(
            [
                ...
                depth_normalized,         # 1
                # ENHANCED: New observations for robustness (3 additional)
                (heading_error_magnitude / torch.pi).unsqueeze(1),  # Normalized heading error
                (heading_error_rate / torch.pi).unsqueeze(1),       # Heading error rate
                alignment_consistency.unsqueeze(1),                 # Alignment consistency metric
            ],
            dim=-1,
        )
        return {"policy": obs}
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # ENHANCED: Update performance tracking
        angle_errors = self._compute_angle_differences()
        good_alignment = (angle_errors < math.radians(10.0)).float()
        self._env_performance += good_alignment
    ...
    def _print_circular_reward_debug(self, rc: Dict[str, torch.Tensor], env_idx: int = 0) -> None:
        ...
        try:
            print("================================")
            print(f"=== ENHANCED CIRCULAR PATH REWARD DEBUG (Env {i}) ===")
            ...
            # **Updated mean reward statistics** after implementing the changes
            mean_reward = rc['total_reward'].mean().item()
            print(f"--- AGGREGATE STATISTICS ---")
            print(f"Mean reward across all envs: {mean_reward:.4f}")
            
            # **Reward variance** across environments - are all improving or just a few?
            reward_std = rc['total_reward'].std().item()
            reward_min = rc['total_reward'].min().item()
            reward_max = rc['total_reward'].max().item()
            print(f"Reward variance (std): {reward_std:.4f}")
            print(f"Reward range: [{reward_min:.4f}, {reward_max:.4f}]")
            
            # Performance consistency check
            rewards_above_threshold = (rc['total_reward'] > 25.0).sum().item()
            consistency_pct = (rewards_above_threshold / self.num_envs) * 100
            print(f"Envs with reward > 25.0: {rewards_above_threshold}/{self.num_envs} ({consistency_pct:.1f}%)")
            
            # ENHANCED: Robustness metrics
            angle_errors = self._compute_angle_differences()
            good_envs = (angle_errors < math.radians(10.0)).sum().item()
            robustness_pct = (good_envs / self.num_envs) * 100
            print(f"Envs with good alignment (<10°): {good_envs}/{self.num_envs} ({robustness_pct:.1f}%)")
            
            # Speed and position distribution
            speed_errors = rc.get('speed_error', torch.zeros_like(rc['total_reward']))
            position_errors = torch.abs(rc['radial_error'])
            print(f"Mean speed error: {speed_errors.mean().item():.4f}")
            print(f"Mean position error: {position_errors.mean().item():.4f}")
            print(f"Mean heading error: {math.degrees(angle_errors.mean().item()):.1f}°")
            
            # **Learning curve trend** tracking
            ...
            # Show trend over last 5 iterations
            if len(self._reward_history) >= 5:
                recent_trend = self._reward_history[-5:]
                trend_slope = (recent_trend[-1] - recent_trend[0]) / 4  # slope per iteration
                print(f"--- LEARNING TREND (Last 5 iterations) ---")
                print(f"Reward trend slope: {trend_slope:+.4f} per iteration")
                print(f"Recent rewards: {[f'{r:.2f}' for r in recent_trend[-3:]]}")
                
                # Determine if learning is progressing
                if trend_slope > 0.1:
                    print("✓ Learning trend: IMPROVING")
                elif trend_slope > -0.1:
                    print("→ Learning trend: STABLE")
                else:
                    print("✗ Learning trend: DECLINING")
            
            print("================================")

            # Add this after direction alignment print:
            print("--- DIRECTION DEBUG ---")
            circle_props = self._get_circle_properties(self._robot.data.root_pos_w)
            
            # AUV body +X direction in world frame
            auv_quat = self._robot.data.root_quat_w[env_idx]
            auv_forward_world = quat_apply(auv_quat.unsqueeze(0), torch.tensor([1.0, 0.0, 0.0], device=self.device).unsqueeze(0))[0]
            
            # Desired tangent in world frame
            desired_tangent = circle_props["desired_tangent_world"][env_idx]
            
            print(f"AUV +X direction (world): ({auv_forward_world[0]:.3f}, {auv_forward_world[1]:.3f})")
            print(f"Desired tangent (world): ({desired_tangent[0]:.3f}, {desired_tangent[1]:.3f})")
            ...
            print(f"Angle difference: {math.degrees(angle_diff.item()):.1f}° (should be ~0°)")

            # ENHANCED: Performance diagnostics every 100 steps
            if self._step_count % 100 == 0:
                poor_envs = self._env_performance < 50.0  # Less than 50% good alignment
                if poor_envs.any():
                    poor_indices = torch.where(poor_envs)[0].tolist()
                    print(f"--- ROBUSTNESS DIAGNOSTIC ---")
                    print(f"Underperforming envs: {poor_indices}")
                    print(f"Consider additional training or parameter adjustment")
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # ENHANCED: Early intervention for excessive drift
        angle_errors = self._compute_angle_differences()
        excessive_drift = angle_errors > math.radians(self.cfg.max_heading_error_degrees)

        out_of_bounds = out_of_bounds | extreme_attitude | too_far_from_circle | excessive_drift
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        # ENHANCED: Reset performance tracking for these environments
        self._env_performance[env_ids] = 0.0
        self._prev_heading_error[env_ids] = 0.0
        ...
        # FIXED: yaw aligned with tangent direction
        # The AUV's +X axis should point in the tangent direction
        desired_yaw = torch.atan2(tan_y, tan_x)
        ...
        # COMPREHENSIVE DEBUG: Verify the alignment calculation
        if self._debug and len(env_ids) > 0:
            i = 0  # Debug first environment
            print(f"=== RESET ALIGNMENT VERIFICATION (Env {env_ids[i]}) ===")
            print(f"Circle position angle: {math.degrees(rand_ang[i].item()):.1f}°")
            print(f"Tangent direction (world): ({tan_x[i]:.3f}, {tan_y[i]:.3f})")
            print(f"Calculated yaw: {math.degrees(desired_yaw[i].item()):.1f}°")
            
            # Step 1: What direction will the AUV's +X axis point after rotation?
            cos_yaw = torch.cos(desired_yaw[i])
            sin_yaw = torch.sin(desired_yaw[i])
            auv_forward_world = torch.tensor([cos_yaw.item(), sin_yaw.item()], device=self.device)
            
            # Step 2: What is the desired tangent direction?
            tangent_world = torch.tensor([tan_x[i].item(), tan_y[i].item()], device=self.device)
            
            # Step 3: Calculate alignment (should be 1.0 for perfect alignment)
            alignment_check = torch.dot(auv_forward_world, tangent_world).item()
            
            print(f"AUV +X will point to: ({auv_forward_world[0]:.3f}, {auv_forward_world[1]:.3f})")
            print(f"Desired tangent direction: ({tangent_world[0]:.3f}, {tangent_world[1]:.3f})")
            print(f"Alignment at reset: {alignment_check:.6f} (target: 1.0)")
            
            # Step 4: Calculate angle difference for clarity
            angle_diff = torch.atan2(
                auv_forward_world[0] * tangent_world[1] - auv_forward_world[1] * tangent_world[0],
                auv_forward_world[0] * tangent_world[0] + auv_forward_world[1] * tangent_world[1]
            )
            print(f"Angle difference: {math.degrees(angle_diff.item()):.1f}° (target: 0.0°)")
            
            if abs(alignment_check - 1.0) > 0.01:
                print("❌ ALIGNMENT ERROR: Reset calculation is incorrect!")
                print("The AUV will not be facing the right direction from the start.")
            else:
                print("✅ ALIGNMENT OK: Reset calculation looks correct")
...
@torch.jit.script
def quat_apply_jit(quat: torch.Tensor, vec: torch.Tensor) -> torch.Tensor:
    # quat: [x, y, z, w], vec: [x, y, z]
...
# ---------- ENHANCED JIT-safe reward for circular path following ----------
@torch.jit.script
...
#    desired_tangent_body_2d = torch.bmm(R_w2b, desired_tangent_world.unsqueeze(-1)).squeeze(-1)
    ...
    # Convert desired tangent to 3D
   tangent_world_3d = torch.zeros(N, 3, device=device, dtype=dtype)
   tangent_world_3d[:, :2] = desired_tangent_world

   # Use quaternion inverse (same as debug calculation)
   quat_inv = torch.stack((-root_quat_w[:, 0], -root_quat_w[:, 1], -root_quat_w[:, 2], root_quat_w[:, 3]), dim=-1)

   # Apply quaternion rotation - you'll need to add this function
   desired_tangent_body_3d = quat_apply_jit(quat_inv, tangent_world_3d)
   desired_tangent_body_2d = desired_tangent_body_3d[:, :2]

   # Direction alignment (BODY +X is forward) - ENHANCED with sign correction
   heading_body_2d = torch.tensor([-1.0, 0.0], device=device, dtype=dtype).repeat(N, 1)
   ...
   # ENHANCED: Stronger direction penalties with exponential drift penalty
   direction_reward = w_direction * direction_alignment
   
   # Add exponential penalty for large angle errors (>30°)
   angle_error = torch.acos(torch.clamp(torch.abs(direction_alignment), 0.0, 1.0))
   drift_penalty = -w_direction * torch.exp(angle_error / 0.5236)  # 30° = 0.5236 rad
   direction_reward = direction_reward + drift_penalty

   # IMPROVED SPEED REWARD: Penalize deviation from target speed
   tangential_speed = torch.sum(root_lin_vel_w[:, :2] * desired_tangent_world, dim=1)
   target_speed_tensor = torch.as_tensor(target_speed, dtype=dtype, device=device)
   
   # Speed error as fraction of target speed
   speed_error = torch.abs(tangential_speed - target_speed_tensor) / (target_speed_tensor + eps)
   
   # Gaussian reward that peaks at target speed and decays with error
   speed_reward = w_speed * torch.exp(-2.0 * speed_error * speed_error)
   
   # Additional penalty for going backwards
   backward_penalty = -0.8 * w_speed * torch.clamp(-tangential_speed, min=0.0) / (target_speed_tensor + eps)
   
   # Severe penalty for excessive overspeed (>2x target)
   overspeed_threshold = 2.0 * target_speed_tensor
   overspeed_penalty = -w_speed * torch.clamp(tangential_speed - overspeed_threshold, min=0.0) / (target_speed_tensor + eps)

   # IMPROVED POSITION REWARD: Tighter tolerance for radial error
   sigma = torch.as_tensor(0.25, dtype=dtype, device=device)  # Reduced from 0.5 to 0.25
   position_reward = w_position * torch.exp(-0.5 * (radial_error / (sigma + eps)) ** 2)
   
   # Additional penalty for being very far from path
   far_penalty = -0.3 * w_position * torch.clamp(torch.abs(radial_error) - 1.0, min=0.0)

   # STABILIZATION REWARDS
   # Roll/pitch stability (assuming upright operation)
   roll = torch.atan2(2.0 * (qw * qx + qy * qz), 1.0 - 2.0 * (qx * qx + qy * qy))
   pitch = torch.asin(torch.clamp(2.0 * (qw * qy - qz * qx), -1.0, 1.0))
   
   attitude_error = torch.sqrt(roll * roll + pitch * pitch)
   upright_reward = w_ang * torch.exp(-2.0 * attitude_error)

   # Angular velocity penalty (reduce oscillations)
   ang_vel_magnitude = torch.norm(root_ang_vel_b, dim=1)
   angvel_penalty = -w_ang_vel * ang_vel_magnitude

   # Action smoothness penalty
   action_magnitude = torch.norm(actions, dim=1)
   action_penalty = -w_actions * action_magnitude

   # ENHANCED: Intermediate correction bonuses for staying aligned
   alignment_bonus = torch.zeros_like(position_reward)
   good_alignment = torch.abs(direction_alignment) > 0.8
   alignment_bonus = torch.where(good_alignment, 0.5 * w_direction, alignment_bonus)

   # Constraint violations (boundaries, extreme attitudes)
   constr_reward = torch.zeros_like(position_reward)
   
   # Alive bonus
   alive_reward = torch.as_tensor(w_alive, dtype=dtype, device=device).repeat(N)

   # ENHANCED: Total reward with robustness improvements
   total_reward = (
       position_reward
       + far_penalty
       + speed_reward
